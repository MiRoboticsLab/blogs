...
## 1. 概述
运动控制(运控)模块类似人的小脑，负责协调各个关节完成规定的动作，比如行走，后空翻，舞蹈等等。运控模块既提供了偏高层的接口，可以让用户直接调用预制的动作单元，也开放了底层的关节电机接口，方便用户开发自己的控制器。运控的接口是基于**LCM**（**Lightweight Communications and Marshalling**）通信，应用板上的ROS上层决策部分也基于本接口构建。

### 1.1 通信框架
机器人整体的通信框架如下图所示：

<p align="center">
  <img width="600" src="./image/cyberdog_loco/flow_chart.png">
</p>
...
<p align="center">
  <img width="800" src="./image/cyberdog_loco/coordinate.png">
</p>
...
```shell
$ git clone git@git.n.xiaomi.com:MiRoboticsLab/loco/loco_hl_example.git # 下载高层接口例程仓库
...
```
### 2.4 接口示例
本节编写了高层接口的3个例程以供参考，例程可以部署在用户电脑或者NX应用板上运行。如果部署在用户电脑，需要按照1.2节所介绍的步骤，安装LCM通信库。所有例程都可以在[例程仓库](https://git.n.xiaomi.com/MiRoboticsLab/loco/loco_hl_example.git)下载。
