...
int32         cmd_type     # 指令类型约束, 1: Data, 2: End；（之前的Start帧可以省略）
...
float32[3]    pos_des      # 当前暂不开放。x y（最大值0.2）z（最大值0.3，待运控确定)  m
...
float32[2]    step_height  # 行走时的抬腿高度，最大值运控组不确定，当前可按0.05m设定
```
...
float32[3]    pos_des      # x y（最大值0.2）z（最大值0.3, 待运控确认)  m
...
float32[2]    step_height  # 抬腿高度，，最大值运控组不确定，当前可按0.05m设定
